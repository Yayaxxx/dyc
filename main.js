 ...
 * - Abonnement temps réel + gardes robustes
 */
...
async function saveCategoriesToFirestore() {
  if (!currentUser) return;
  try {
    await setDoc(doc(db, "users", currentUser), { categories }, { merge: true });
  } catch (err) {
...
async function registerUser() {
  ...
  try {
    ...
    categories = [...DEFAULT_CATEGORIES];
    await setDoc(doc(db, "users", currentUser), { categories });
...
async function saveItem() {
  ...
  try {
    if (editingItemPath) {
      await setDoc(docFromPath(editingItemPath), itemData);
    } else {
...
async function deleteItem(itemPath) {
  ...
  try {
    await deleteDoc(docFromPath(itemPath));
  } catch (err) {
  ...
  if (currentUser) {
    inventory.forEach(item => {
      if (item.category === oldName && item.path?.startsWith(`users/${currentUser}/items/`)) {
        updateDoc(docFromPath(item.path), { category: newName })
          .catch(err => console.error("Erreur maj catégorie article :", err));
      }
    });
  }
